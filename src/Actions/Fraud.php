...
class Fraud implements ActionInterface {
	...
	public function validate_params($parameters){
		...
		$required_class_names = [
			...
			'Buonzz\Epoch\CustomerSearch\Parameters\Fraud'
		];
		...
		if(count($parameters)<2 || count($present_params) < 2 )
			throw new \InvalidArgumentException("Epoch's Search API requires member_id and fraud parameters for fraud action");
